...
volatile struct fs20_queue_t fs20_queue;
...
void fs20_send(uint8_t fht, uint16_t housecode, uint8_t address,
               uint8_t command, uint8_t command2) {
  for (uint8_t i = 0; i < 3; i++) {
    fs20_send_sync();

    /* magic constant 6 from fs20 protocol definition, 0x0c from fht protocol
     * definition... */
...
ISR(ANALOG_COMP_vect) {
  ...
  if (fs20_global.fs20.timeout == 0 &&
      ((fs20_global.fs20.rec < FS20_DATAGRAM_LENGTH) ||
       ((fs20_global.fs20.datagram.data.edg.cmd & _BV(5)) &&
    ...
    /* check value */
    int8_t v;
    if (FS20_PULSE_ZERO(time) && FS20_PULSE_ZERO(time_old) &&
        FS20_PULSE_DIFFERENCE(time, time_old)) {
      /* we received a zero */
      v = 0;
    } else if (FS20_PULSE_ONE(time) && FS20_PULSE_ONE(time_old) &&
               FS20_PULSE_DIFFERENCE(time, time_old)) {
      /* we received a one */
      v = 1;
    } else {
    ...
    if (!fs20_global.fs20.sync) {
      ...
      if (v >= 0) {
        ...
        time_old = 0;
        memset((void *)&fs20_global.fs20.datagram, 0,
               sizeof(fs20_global.fs20.datagram));
  ...
  if (fs20_global.ws300.rec < FS20_WS300_DATAGRAM_LENGTH) {
    ...
    if (!fs20_global.ws300.sync) {
      ...
      if (v >= 0) {
        ...
        fs20_global.ws300.null = 0;
        memset((void *)&fs20_global.ws300.datagram, 0,
               sizeof(fs20_global.ws300.datagram));
      } else {
        time_old += time;
...
#endif
{
  ...
  if (fs20_global.ws300.rec != FS20_WS300_DATAGRAM_LENGTH) {
    ...
    fs20_global.ws300.null = 0;
    memset((void *)&fs20_global.ws300.datagram, 0,
           sizeof(struct ws300_datagram_t));
  }
#endif
}
...
void fs20_process(void) {
  ...
  if ((fs20_global.fs20.rec == FS20_DATAGRAM_LENGTH && !extbit) ||
    ...
    if (fs20_global.fs20.rec == FS20_DATAGRAM_LENGTH) {
      ...
      for (uint8_t i = 0; i < 9; i++) {
        for (int8_t b = 7; b >= 0; b--) {
          printf("%u",
                 (fs20_global.fs20.datagram.data.bytes[i] & (1 << b)) > 0);
        }
        printf(" ");
      }
      printf("\n");

      debug_printf("1cmd: sync %02x hc1 %02x p1 %u hc2 %02x p2 %u addr %02x "
                   "p3 %u cmd %02x p4 %u parity %02x p5 %u\n",
...
#else
      debug_printf(
          "fs20 recv: %02x%02x %02x %02x (sync: %u)\n",
          fs20_global.fs20.datagram.data.dg.hc1,
          fs20_global.fs20.datagram.data.dg.hc2,
          fs20_global.fs20.datagram.data.dg.addr,
          fs20_global.fs20.datagram.data.dg.cmd,
      ...
      debug_printf("2cmd: sync %02x hc1 %02x p1 %u hc2 %02x p2 %u addr %02x "
                   "p3 %u cmd %02x p4 %u cmd2 %02x p5 %u parity %02x p6 %u\n",
...
#else
      debug_printf("fs20 recv ext: %02x%02x %02x %02x %02x "
                   "(sync: %u)\n",
                   fs20_global.fs20.datagram.data.edg.hc1,
                   fs20_global.fs20.datagram.data.edg.hc2,
                   fs20_global.fs20.datagram.data.edg.addr,
    ...
    for (uint8_t l = 0; l < fs20_queue.len; l++) {
      if (fs20_queue.queue[l].isfs20) {
        struct fs20_datagram_t *dg = &fs20_queue.queue[l].fs20;
        if (dg->ext) {
          debug_printf("%u: %02x%02x %02x %02x %02x\n", l, dg->data.edg.hc1,
                       dg->data.edg.hc2, dg->data.edg.addr, dg->data.edg.cmd,
                       dg->data.edg.cmd2);

        } else {
          debug_printf("%u: %02x%02x %02x %02x\n", l, dg->data.dg.hc1,
                       dg->data.dg.hc2, dg->data.dg.addr, dg->data.dg.cmd);
        }
      } else {
        debug_printf(
            "%u: %d.%u %u %u.%u %u %u", l, fs20_queue.queue[l].ws300.temp,
            fs20_queue.queue[l].ws300.temp_frac,
            fs20_queue.queue[l].ws300.hygro, fs20_queue.queue[l].ws300.wind,
            fs20_queue.queue[l].ws300.wind_frac,
            fs20_queue.queue[l].ws300.rain,
            fs20_queue.queue[l].ws300.rain_value);
      }
    }
#endif
#endif
    ...
    if (fs20_global.fs20.datagram.data.dg.sync == 0x0001) {
      ...
      fs20parity = dg->data.dg.hc1 + dg->data.dg.hc2 + dg->data.dg.addr +
                   dg->data.dg.cmd;
      ...
      if (!p1 && !p2 && !p3 && !p4 && !p5 && !p6 &&
          (fs20parity == dgparity || fhtparity == dgparity)) {
        ...
        memmove(&fs20_queue.queue[1], &fs20_queue.queue[0],
                (FS20_QUEUE_LENGTH - 1) *
                    sizeof(struct fs20_queue_element_t));

        /* copy datagram to queue */
        memcpy(&fs20_queue.queue[0].fs20,
               (const void *)&fs20_global.fs20.datagram,
               sizeof(struct fs20_datagram_t));

        fs20_queue.queue[0].isfs20 = 1;
        fs20_queue.queue[0].send = 1;

        if (fs20_queue.len < FS20_QUEUE_LENGTH)
          fs20_queue.len++;
      }
#ifdef DEBUG_FS20_REC
      else {
        debug_printf("fs20: invalid datagram\n");
      }
#endif
    }
#ifdef DEBUG_FS20_REC
    else {
      debug_printf("fs20: sync invalid: %04x\n",
                   fs20_global.fs20.datagram.data.dg.sync);
    }
#endif
...
void ws300_parse_datagram(void) {
  ...
  if (!(d->p1 && d->p2 && d->p3 && d->p4 && d->p5 && d->p6 && d->p7 &&
        d->p8 && d->p9 && d->p10 && d->p11 && d->p12 && d->p13 && d->p14 &&
        d->p15)) {
#ifdef DEBUG_FS20_WS300
    debug_printf("ws300: checksums: at least one marker is not 1!\n");
#endif
    return;
  }
...
#ifdef DEBUG_FS20_WS300
    debug_printf("ws300: invalid constant: %02x!\n", d->constant);
#endif
    return;
  }
  ...
  xor = d->constant ^ d->flags ^ d->temp1 ^ d->temp2 ^ d->temp3 ^ d->hygro1 ^
        d->hygro2 ^ d->wind1 ^ d->wind2 ^ d->wind3 ^ d->raincounter1 ^
        d->raincounter2 ^ d->raincounter3 ^ d->unknown ^ d->checksum1;

  sum = d->constant + d->flags + d->temp1 + d->temp2 + d->temp3 + d->hygro1 +
        d->hygro2 + d->wind1 + d->wind2 + d->wind3 + d->raincounter1 +
        d->raincounter2 + d->raincounter3 + d->unknown + d->checksum1;
  ...
  if (xor != 0) {
#ifdef DEBUG_FS20_WS300
    debug_printf("ws300: invalid checksum1!\n");
#endif
  ...
  if (sum != d->checksum2) {
#ifdef DEBUG_FS20_WS300
    debug_printf("ws300: invalid checksum2: %u != %u!\n", sum, d->checksum2);
#endif
  ...
  fs20_global.ws300.data.temp_frac = d->temp1;
  fs20_global.ws300.data.temp = d->temp2 + 10 * d->temp3;

  fs20_global.ws300.data.rain = (d->flags & _BV(FS20_WS300_FLAG_WATER)) > 0;

  if (d->flags & _BV(FS20_WS300_FLAG_TEMP))
    fs20_global.ws300.data.temp = -fs20_global.ws300.data.temp;

  fs20_global.ws300.data.hygro = d->hygro1 + 10 * d->hygro2;

  fs20_global.ws300.data.wind_frac = d->wind1;
  fs20_global.ws300.data.wind = d->wind2 + 10 * d->wind3;

  /*
      fs20_global.ws300.data.rain_value = d->raincounter1
          + 10 * d->raincounter2
          + 100 * d->raincounter3;
  */

  fs20_global.ws300.data.rain_value =
      (d->raincounter1) | (d->raincounter2 << 4) | (d->raincounter3 << 8);

  /* reset update counter */
  fs20_global.ws300.last_update = 0;

#ifdef DEBUG_FS20_WS300
  debug_printf("ws300: %d.%u deg, %u%% hygro, %u.%u km/h wind, "
               "raincount: %u, rain: %u\n",
               fs20_global.ws300.data.temp, fs20_global.ws300.data.temp_frac,
               fs20_global.ws300.data.hygro, fs20_global.ws300.data.wind,
               fs20_global.ws300.data.wind_frac,
               fs20_global.ws300.data.rain_value,
               fs20_global.ws300.data.rain);
#endif

  /* shift queue backwards */
  memmove(&fs20_queue.queue[1], &fs20_queue.queue[0],
          (FS20_QUEUE_LENGTH - 1) * sizeof(struct fs20_queue_element_t));

  /* copy datagram to queue */
  memcpy(&fs20_queue.queue[0].ws300, (const void *)&fs20_global.ws300.data,
         sizeof(struct ws300_data_t));

  fs20_queue.queue[0].isfs20 = 0;
  fs20_queue.queue[0].send = 1;
...
void fs20_init(void) {
  ...
  memset((void *)&fs20_global, 0, sizeof(struct fs20_global_t));
  memset((void *)&fs20_queue, 0, sizeof(struct fs20_queue_t));
